...
the catmaid package and the NeuroAnatomy Toolbox ([nat](https://jefferis.github.io/nat/))
on which it builds.
...
We can also add some useful metdata. 
```{r orn-meta}
...
olfactory sensory neurons. These two variables are stored as columns in a table (i.e. `data.frame()`) that is an attribute of the `orns` `neuronlist` objtect.

The *catmaid* package builds on [nat](https://jefferis.github.io/nat), a general
purpose R neuroanatomy package. You may want to read the [nat documentation](https://jefferis.github.io/nat/articles/neurons-intro.html) to
...
Plot result of clusterting as dendrogram, labelled by abbreviated 
